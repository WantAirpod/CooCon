...
var
  xParkingWindow: HWND;
  ParkingWndList : TThreadList; // bug-fix2

  { // bug-fix start }
  procedure pkw_FreeAllParkingWnd();
  var
	lst : TList;
	i : integer;
    pd : TParkingWndData; 
  begin
	lst := ParkingWndList.LockList;
	if(lst<>nil) then
	begin
		for i:=0 to (lst.count-1) do
		begin
			pd := TParkingWndData(lst.Items[i]);
			if (pd<>nil) then
			begin
				if ( IsWindow(pd.hPakingWnd)=true) then
					SendMessage(pd.hPakingWnd, WM_CLOSE, 0, 0);
				pd.Free;
			end;
		end; // end for
	...
	ParkingWndList.UnlockList;
  end;

  // Find Parking Window /////////////////////////////////////
  function pkw_FindParkingWindow() : HWND;
  var
	lst : TList;
	i : integer;
	pd : TParkingWndData;
  ...
  begin
	...
	begin
		...
		begin
			...
			if (pd<>nil) then
			begin
				if ( IsWindow(pd.hPakingWnd)=false) then
				begin
					pd.Free;
					lst.Delete(i);
				end
				...
				begin
					if(pd.dwThreadID = dwThreadID) then
					begin
						Result := pd.hPakingWnd;
						break;
					end;
				end;
			end;
  ...
  // add Parking Window /////////////////////////////////////
  procedure pkw_AddParkingWindow(hwnd : HWND);
  var
	lst : TList;
	pd : TParkingWndData;
	dwThreadID : DWORD;
  begin
	...
	begin
		...
		pd := TParkingWndData.Create();
		pd.dwThreadID := dwThreadID;
		pd.hPakingWnd := hwnd;

		ParkingWndList.Add(pd);
...
begin
 // Result := xParkingWindow;
  ...
  Result := pkw_FindParkingWindow();
  
  if Result <> 0 then Exit;

  // bug-fix : fix Dax error : accessviolation (win2k, win xp)
  ParkingName := 'DAXParkingWindow_' +   Format('%p', [@ParkingWindowProc]);

  FillChar(TempClass, sizeof(TempClass), 0);
  if not GetClassInfo(HInstance, PChar(ParkingName), TempClass) then // bug-fix : fix Dax error : accessviolation (win2k, win xp)
  begin
    TempClass.hInstance := HInstance;
    TempClass.lpfnWndProc := @ParkingWindowProc;
    TempClass.lpszClassName := PChar(ParkingName); // bug-fix : fix Dax error : accessviolation (win2k, win xp)
    if Windows.RegisterClass(TempClass) = 0 then
      raise EOutOfResources.Create(SWindowClass);
  end;

  ParkingWndList.LockList; // bug-fix
    
  hPark := CreateWindowEx(WS_EX_TOOLWINDOW, TempClass.lpszClassName, nil,
    WS_POPUP, GetSystemMetrics(SM_CXSCREEN) div 2,
    GetSystemMetrics(SM_CYSCREEN) div 2, 0, 0, 0, 0, HInstance, nil);
  SetWindowPos(hPark, 0, 0, 0, 0, 0, SWP_NOACTIVATE or SWP_NOREDRAW
	  or SWP_NOZORDER or SWP_SHOWWINDOW);

  pkw_AddParkingWindow(hPark);

  ParkingWndList.UnlockList; // bug-fix
...
initialization
  ...
  InitOlePro32;
  ParkingWndList := TThreadList.Create; // bug-fix2  
...
finalization
  ...
  if (ParkingWndList <> nil) then; // bug-fix2
  begin
	  pkw_FreeAllParkingWnd();
	  ParkingWndList.Free;
  end;
