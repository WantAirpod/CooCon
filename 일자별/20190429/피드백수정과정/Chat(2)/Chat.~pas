...
var
  Form1: TForm1;
  UserList :array[0..100] of String;  //유저의 아이디를 담는 배열 생성
  stlTemp : TStringList;
implementation
type
...
begin
 ...
 stlTemp.Text:= StringReplace(stTemp, ':' , #13 , [rfReplaceAll] );  // :를 #13으로 변환
 stReceivedText:= AThread.Connection.ReadLn('',1);   // ReadLn 짧게 처리 함-> 바로바로 서버에서 받아옴
  If stReceivedText = '' then exit;    //받아오는 값이 없다면 exit
 //채팅 처리
 ...
 Try
  For Loop:= 0 to List.Count-1 do Begin      //클라이언트 연결 수 만큼 반복을하면서 모든 연결된 클라이언트에 메시지를 전송
     ...
     Try
       IdPeerThread.Connection.WriteLn(stReceivedText);    //Client에게 뿌려주는 코드
     Except
       IdPeerThread.Stop;
     End;
   End;
 Finally
   IdTCPServer1.Threads.UnlockList;  //락 해제
 End;
//로그인 처리
if (pos('로그인',stReceivedText) <> 0) And (pos('talk',stReceivedText)=0) then    //Text에서 로그인 이라는 단어를 찾으면...
begin
 if UserList[0]='' then
